...
const ai_tracking_service_1 = require("./ai-tracking.service");
const logger_1 = require("../utils/logger");
const constants_1 = require("../config/constants");
class ScanService {
    constructor() {
        this.scanQueue = new bull_1.default(constants_1.QUEUE_NAMES.WEBSITE_SCAN, {
            redis: {
                host: process.env.REDIS_HOST || 'localhost',
                port: parseInt(process.env.REDIS_PORT || '6379')
            }
        });
        this.crawlerService = new crawler_service_1.CrawlerService();
        this.optimizationService = new optimization_service_1.OptimizationService();
        this.aiTrackingService = new ai_tracking_service_1.AITrackingService();
    ...
    setupQueueProcessors() {
        this.scanQueue.process('website-scan', async (job) => {
            ...
            try {
                logger_1.logger.info(`Starting scan ${scanId} for website ${websiteId}`);
                // Update scan status to running
                await this.updateScanStatus(scanId, constants_1.SCAN_STATUS.RUNNING, 0);
                // Step 1: Crawl website (30% progress)
                await job.progress(10);
                const crawlData = await this.crawlerService.crawlWebsite(await this.getWebsiteUrl(websiteId), scanType);
                ...
                logger_1.logger.info(`Completed scan ${scanId} with GEO score: ${geoScore}`);
                return results;
            }
            catch (error) {
        ...
        this.scanQueue.on('completed', (job, result) => {
            logger_1.logger.info(`Scan job ${job.id} completed successfully`);
        });
        this.scanQueue.on('failed', (job, error) => {
            logger_1.logger.error(`Scan job ${job.id} failed:`, error);
        });
        this.scanQueue.on('progress', (job, progress) => {
            logger_1.logger.debug(`Scan job ${job.id} progress: ${progress}%`);
        });
    }
    async initiateScan(websiteId, scanType = constants_1.SCAN_TYPES.STANDARD, userId) {
        try {
            ...
            const existingScan = await models_1.Scan.findOne({
                where: {
                    websiteId,
                    status: [constants_1.SCAN_STATUS.PENDING, constants_1.SCAN_STATUS.RUNNING]
                }
            });
            if (existingScan) {
            ...
            // Create scan record
            const scan = await models_1.Scan.create({
                websiteId,
                scanType: scanType,
                status: constants_1.SCAN_STATUS.PENDING,
                progress: 0
            });
            // Add job to queue
            ...
            logger_1.logger.info(`Initiated scan ${scan.id} for website ${websiteId}`);
            return scan.id;
        }
        catch (error) {
            logger_1.logger.error('Failed to initiate scan:', error);
            throw error;
        }
    }
    async getScanStatus(scanId) {
        const scan = await models_1.Scan.findByPk(scanId, {
            include: [{ model: models_1.Website, as: 'website' }]
        });
        if (!scan) {
    ...
    async getScanResults(scanId) {
        const scan = await models_1.Scan.findByPk(scanId);
        if (!scan || scan.status !== constants_1.SCAN_STATUS.COMPLETED) {
            return null;
        }
        return scan.results;
    }
    async getWebsiteUrl(websiteId) {
        const website = await models_1.Website.findByPk(websiteId);
        if (!website) {
            throw new Error('Website not found');
        }
    ...
    async updateScanStatus(scanId, status, progress) {
        const updateData = { status };
        if (status === constants_1.SCAN_STATUS.RUNNING && !await this.getScanStartTime(scanId)) {
            updateData.startedAt = new Date();
        }
        if (progress !== undefined) {
            updateData.progress = progress;
        }
        await models_1.Scan.update(updateData, { where: { id: scanId } });
    }
    async getScanStartTime(scanId) {
        const scan = await models_1.Scan.findByPk(scanId, { attributes: ['startedAt'] });
    ...
    calculateGEOScore(analysis) {
        const weights = constants_1.GEO_SCORING_WEIGHTS;
        let score = 0;
    ...
    calculateStructureQuality(crawlData) {
    ...
    calculateCitations(crawlData) {
        // This would analyze external authoritative links, references, etc.
        // For now, use external links as a proxy
        let totalExternalLinks = 0;
        crawlData.forEach(page => {
            totalExternalLinks += page.links.filter(link => link.type === 'external' &&
                this.isAuthoritativeSource(link.href)).length;
        });
        return Math.min(100, totalExternalLinks * 10);
    }
    calculateExpertise(crawlData) {
    ...
    analyzeTechnicalIssues(crawlData) {
        const issues = [];
    ...
    calculatePerformanceMetrics(crawlData) {
    ...
    async saveContentData(websiteId, crawlData, geoScore) {
        for (const page of crawlData) {
            try {
                // Check if content already exists
                let content = await models_1.Content.findOne({
                ...
                const contentData = {
                    ...
                    schemaTypes: page.schema.map((s) => s['@type']).filter(Boolean),
                ...
                if (content) {
                    ...
                    await models_1.Content.create(contentData);
                }
            }
            catch (error) {
                logger_1.logger.error(`Failed to save content for ${page.url}:`, error);
            }
        }
    }
    async completeScan(scanId, results) {
        await models_1.Scan.update({
            status: constants_1.SCAN_STATUS.COMPLETED,
            progress: 100,
    ...
    async failScan(scanId, errorMessage) {
        await models_1.Scan.update({
            status: constants_1.SCAN_STATUS.FAILED,
            errorMessage
        }, { where: { id: scanId } });
    }
    estimateTimeRemaining(progress, startedAt) {
