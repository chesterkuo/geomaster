...
import { apiKeyManager } from '../apiKeyManagerService';
...
export class AITrackingPlatformFactory implements PlatformFactory {
  ...
  // Get platform with configuration based on user/env API keys
  async createPlatformWithApiKeys(platformName: string, organizationId: string): Promise<AITrackingPlatform> {
    const apiKeyConfig = await apiKeyManager.getApiKeyForPlatform(platformName, organizationId);

    if (!apiKeyConfig) {
      throw new PlatformError(
        platformName,
        'NO_API_KEY',
        `No API key available for platform ${platformName}`
      );
    }
